# I'm Aamir

**Lahore** · **Founder [CalendarJet](https://calendarjet.com)** · **Featured on [Semrush][semrush-article] & [Ahrefs][ahrefs-mention]** · **10 Years in SEO**

[![SEO][badge-seo]][x]
[![TypeScript][badge-ts]][x]
[![React][badge-react]][x]
[![Next.js][badge-next]][x]
[![Node.js][badge-node]][x]
[![Supabase][badge-supabase]][x]
[![Tailwind CSS][badge-tailwind]][x]
[![Python][badge-python]][x]
[![Cloudflare][badge-cloudflare]][x]
[![Vercel][badge-vercel]][x]

> 10 years of SEO. Now I build the tools. I don't just write code — I build for the people I've spent a decade studying.

I spent 10 years ranking websites, studying how millions of people search, click, and behave online. Grew sites from 20K to 528K pageviews/month. Led SEO at **[Cryptonary][cryptonary]** (London) and **[One Smart Sheep][oss]** (Maryland). Then I started building.

#### 🗓️ **[CalendarJet](https://calendarjet.com)** – Scheduling infrastructure for the AI era. Your AI agent modifies availability. Custom domains day one.

## Projects

> Real users, real URLs. Tools built by someone who's studied millions of user journeys.

- 🔗 **[upload.nafran.com][upload]** – Drop a file, get a link. Up to 1GB, auto-deletes in 30 min. CLI support. [[Source][upload-src]] `React 19` `TanStack Start` `Vercel Blob`
- 🌐 **[host.nafran.com][host]** – Drop HTML or paste code, get instant hosting. Zero signup. [[Source][host-src]] `React` `Supabase` `Cloudflare Pages`
- 📝 **[counter.nafran.com][counter]** – Word counter & text analysis for writers. Reading level, keyword density, AI token estimation. [[Source][counter-src]] `Pure HTML/CSS/JS` `Zero dependencies`
- 🧹 **[Website HTML Extractor][extractor]** – Extract clean HTML from any website. Strips JS, CSS, scripts. Keeps structure and meta. [[Source][extractor]]
- 🎬 **[yt-web-downloader][ytdl]** – YouTube downloader with modern UI. 1000+ platforms. [[Source][ytdl]] `Next.js` `Framer Motion` `yt-dlp`
- 📜 **[Vimeo Transcript][vimeo]** – Extract and download transcripts from Vimeo videos. [[Source][vimeo]]

<details>
<summary><b>More projects</b></summary>
<br>

- 📅 **[calendar-pro][calpro]** – Scheduling prototype. TypeScript, Vercel
- 🔗 **[linkforge][linkforge]** – Link management tool. TypeScript

</details>

## The SEO Chapter

Before code, I spent 10 years in the SEO trenches. Some highlights:

**Press & Publications**
- 📰 **[Semrush Blog][semrush-article]** — *"Google NLP API Tool: Optimize Your Content to the Next Level"* — world's most-read SEO blog
- 📰 **[ProPakistani][propakistani-article]** — *"Buying a Laptop? Keep These 9 Useful Tips in Mind"* — Pakistan's #1 tech publication
- 📰 **[Dawn][dawn-article]** — Pakistan's most widely-read English newspaper
- 📰 **[Ahrefs][ahrefs-mention]** — Featured on Ahrefs Instagram

**Results**
- 📈 **20K → 528K pageviews/month** — Top Study World, scaled over 7 years
- 📈 **105K clicks** · **8.72M impressions** in 12 months — One Smart Sheep
- 📈 **101K clicks** · **3.3% CTR** for eCommerce — Alita Accessories
- 🏆 **Hult Prize Semi-final Winner** — Top 15 out of 139 business ideas

**Roles**
- **Head of SEO** @ [Cryptonary][cryptonary] — Technical audits on Next.js, bulk ops with OpenAI API + Screaming Frog + Ahrefs API across 5000+ articles
- **SEO Specialist** @ [One Smart Sheep][oss] — WordPress content at Semrush blog standards, 105K clicks in 12 months
- **SEO Manager** @ Alita Accessories — eCommerce category ranking, 101K clicks
- **SEO Specialist** @ Top Study World — From scratch to 528K/mo, 7 years

## How I Think

Most developers build features. I build for **behavior**.

10 years of search data teaches you what people click, what they ignore, and what makes them leave in 3 seconds.

**CalendarJet** exists because scheduling tools failed in the AI era.
**upload.nafran.com** exists because sharing a file should take 2 seconds, not 20.
**host.nafran.com** exists because deploying HTML shouldn't need a CI/CD pipeline.

Simple tools. Zero friction. Built by someone who actually understands users.

---

<p>
  <a href="https://nafran.com">nafran.com</a> ·
  <a href="https://calendarjet.com">calendarjet.com</a> ·
  <a href="https://linkedin.com/in/aamirmursleen">LinkedIn</a>
</p>

<!-- Badge definitions -->
[badge-seo]: https://img.shields.io/badge/SEO-10%20Years-0A66C2?style=flat-square&logo=google&logoColor=white
[badge-ts]: https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white
[badge-react]: https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black
[badge-next]: https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=nextdotjs&logoColor=white
[badge-node]: https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white
[badge-supabase]: https://img.shields.io/badge/Supabase-3FCF8E?style=flat-square&logo=supabase&logoColor=white
[badge-tailwind]: https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white
[badge-python]: https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white
[badge-cloudflare]: https://img.shields.io/badge/Cloudflare-F38020?style=flat-square&logo=cloudflare&logoColor=white
[badge-vercel]: https://img.shields.io/badge/Vercel-000000?style=flat-square&logo=vercel&logoColor=white
[x]: #

<!-- Links -->
[semrush-article]: https://www.semrush.com/blog/google-nlp-api/
[propakistani-article]: https://propakistani.pk/2018/09/04/buying-a-laptop-keep-these-9-useful-tips-in-mind/
[ahrefs-mention]: https://www.instagram.com/p/B41qWR6ppNZ/?hl=en&img_index=7
[dawn-article]: https://www.dawn.com/news/1203026
[cryptonary]: https://cryptonary.com
[oss]: https://onesmartsheep.com

[upload]: https://upload.nafran.com
[upload-src]: https://github.com/aamirmursleen/file-share
[host]: https://host.nafran.com
[host-src]: https://github.com/aamirmursleen/quick-share-html
[counter]: https://counter.nafran.com
[counter-src]: https://github.com/aamirmursleen/wordcounter2
[extractor]: https://github.com/aamirmursleen/Website-HTML-Extractor
[vimeo]: https://github.com/aamirmursleen/download-vimeo-transcript
[ytdl]: https://github.com/aamirmursleen/yt-web-downloader
[calpro]: https://calendar-pro-rosy.vercel.app
[linkforge]: https://github.com/aamirmursleen/linkforge
